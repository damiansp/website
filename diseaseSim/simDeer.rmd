...
# Warnings are turned off here so that they will not appear in the final
# knitted html file.  Omit this line when running locally.
options(warn = -1)
...
# When knitting, animalPlot has to be called again (ignore if running locally)
animalPlot(xSummer, ySummer,
		   main = paste('Initial Distribution', 
		   				'Winter, Prior to Dispersal or Migration', 
		   				sep = '\n'),
...
# If knitting recreate animalPlot() first. (Ignore if running locally)
animalPlot(xWinter, yWinter)
...
```
<br />
Newborns are shown in small yellow and green points, and should all share coordinates with their mothers.
...
# Step 10: Diminish Infectious Content in Soil	D
...
# Don't forget to turn warnings back on!
options(warn = 0)
```