...
<ul>
<li>Typically there are more FAs present in the sample than in the standards, so either many standards must be obtained, or the researcher must be content with having some FAs remaining unidentified.</li>
<li>If two or more FAs have ranges of retention times that overlap, there is no way to identify which molecule is which with absolute certainty, on the basis of the GC alone.</li>
<li>Impurities will typically be present in the sample, causing the GC to identify a molecule which is not a FA at all.</li>
</ul>
...
<ol>
<li>For all FA with known identities, determine their median retention time across all samples, and use the median values to calibrate samples such that the calibrated retention times match exactly for the known FAs.</li>
<li>Use the retention times of the known FAs as one set of centroids that will only be allowed to vary slightly in value.</li>
<li>Because there are more FAs in the samples than the known ones, we add additional centroids, with randomly intialized values (uniform over the range of observed retention times).  There are two approaches that might be taken here: 
	<ul>
	<li>Optimize to find the number of centroids that best explains the variation</li>
	<li>Set an fixed number of additional cetroids.</li>
	</ul>
Because the optimization approach led to predictions of many fewer FAs than we knew to be present based on existing literature, we ended up going with a fixed number, also based on the literature.</li>
<li>Using the set of centroids, run the k-means algorithm and determine the amount of variance explained by the groupings (sum of squares between clusters / total sum of squares).</li>
<li>Randomly reinitialize centroids and perform k-means again.  If new clusters do a better job of explaining the variance, save this group of clusters instead.</li>
<li>Repeat for an arbitrary number of iterations, saving the best clusters any time they outperform the existing saved set of clusters.</li>
</ol>
...
bestClust = function(data, tot.centers, tot.iter, minVal, maxVal, 
	...
	best.etas = numeric(tot.iter + 1)
	k.out = 0				  	
	class(k.out) = 'try-error'
	colors = character(tot.centers)
	for (i in 1:tot.centers) {
	...
	for (i in 1:tot.iter) {
		...
		best.etas[i + 1] = best.eta
	}

	plotCols = colors[this.out$cluster]
	plot(data, col = plotCols, pch = 16, cex = 0.7)
	abline(h = spec.centers)
	return (list(clustering = this.out, best.etas = best.etas))
}

random.color = function(tmax = 2) {
	r = runif(1, 0, 1)
	g = runif(1, 0, 1)
	# keep color from being too light to be visible
	b = runif(1, 0, min(1, tmax - r - g)) 
	return (rgb(r, g, b))
}
...
In practice, even more iterations might be tried, as there is no guarantee that the optimal solution has been found.  Better still would be save and plot the best eta value (the metric indicating the goodness of fit) after each iteration, and determine the rate at which new best solutions are being found.

```{r etas}
plot(clust_60FA$best.etas, type = 'l', col = 2, xlab = 'Iterations', 
	 ylab = expression(eta))
```
This output shows that the eta value quickly improves, but there may be room for further improvement yet with more iterations, which I omit here.
...
assignments60FA = data.frame(
	scaledMRT = as.vector(as.matrix(fat[,mrts][fat[,mrts] > 0])),
	cluster = clust_60FA$clustering$cluster
)

head(assignments60FA)
